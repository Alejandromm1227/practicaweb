...
function manejarEjercicio4() {
    const $pptResultado = d.getElementById("pptResultado");    
    ...
    opcionesUsuario.forEach((opcion)=>{
        opcion.addEventListener("click", ()=>{
            ...
            const resultado = jugar(seleccionUsuario);
            if (resultado != "empate") {
                finalizarJuego = true;
            }
            $pptResultado.textContent = resultado;
        })
    })
}

function jugar(seleccionUsuario) {
    if (finalizarJuego) return "el juego ha finalizado"
    ...
    if (seleccionMaquina === seleccionUsuario) {
        ...
        if (seleccionUsuario === "piedra" && seleccionMaquina === "papel" ||
        seleccionUsuario === "papel" && seleccionMaquina === "tijera" ||
        seleccionUsuario === "tijera" && seleccionMaquina === "piedra"
        ) {
